...
def definition(operation_id, method, path, operation):
    ...
    return {
        ...
        "queryParams": [
            {"name": p["name"], **({"collectionFormat": p["collectionFormat"]} if "collectionFormat" in p else {})}
            for p in params
            if p.get("in") == "query"
        ],
        "formParams": [
            {"name": p["name"], **({"type": p["type"]} if "type" in p else {})}
            for p in params
...
def main():
    ...
    for path, methods in sorted(spec["paths"].items()):
        for method, operation in sorted(methods.items()):
            ...
            groups[group_name][alias(operation_id, tag, used_by_group[group_name])] = operation_id
